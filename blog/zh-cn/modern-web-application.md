...
- apigateway的共享实例是按照流量和api调用计费，例如API总调用次数为1200万次，同时产生了100GB的公网出访流量，因此在这个月内需要共需要支付144元，包括：API调用量（次数）费用为1000*0.06+100*0.04=64元，其中计费次数为1100万次；流量费用为100*0.8=80元。
这个调用次数和访问流量已经非常夸张了
- fc 按照调用次数（0.0133元/万次），函数实例使用量（0.000110592元/GB-秒），公网出流量（函数内数据传输流量：0.80元/GB，函数请求响应流量：0.50元/GB，CDN回源流量：0.50元/GB），每月有价值约46元的免费额度。按照调用100W次，执行内存512M,请求平均时间600ms算每个月都是免费的
- oss 本身主要也是按照存储和公网流量计费（额外服务要收费），香港和海外可以免费存5个G对于展示类站点足够，OSS计算流量费用时，分为忙时流量（8:00~24:00，0.50元/GB）和闲时流量（00:00~08:00，0.25元/GB）。100G的流量 * 0.5 差不多50块，如果开通apigateway的专项实例走内网，这部分也不收钱。
- dns 个人版（48元一年）
